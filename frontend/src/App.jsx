...
function ProductShell({ onLogout }) {
        ...
        <div className="app-layout">
            <aside className="sidebar">
                ...
                <nav className="sidebar-nav">
                    ...
                    <NavLink to="/app/data" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/targets" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/suppliers" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/simulator" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/auditor" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/benchmarks" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" /></svg>
                        Benchmarks
                    </NavLink>
                    <NavLink to="/app/budget" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M16 8h-6a2 2 0 100 4h4a2 2 0 010 4H8" /><path d="M12 18V6" /></svg>
                        Carbon Budget
                    </NavLink>
                    <NavLink to="/app/compliance" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
                        Compliance
                    </NavLink>
                    <NavLink to="/app/board-report" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                    ...
                    <NavLink to="/app/integrations" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                ...
                <div className="sidebar-footer">
                    {user && user.id !== 'guest' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', fontSize: '0.8rem' }}>
                            ...
                            <button className="btn-ghost" onClick={onLogout} style={{ padding: '4px', fontSize: '12px' }} title="Sign out">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
                            </button>
                        </div>
                    )}
                    {user && user.id === 'guest' && (
                        <button className="btn-ghost" onClick={onLogout} style={{ fontSize: '0.75rem', marginBottom: '12px', width: '100%', textAlign: 'left' }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
                            &nbsp;Sign Out
                        </button>
                    )}
                    <div className="sidebar-status" style={{ marginBottom: '12px' }}>
                        ...
                        {error ? 'Connection Error' : 'MCP + Postgres Connected'}
                    </div>
...
function SignupPage() {
    ...
    return <Login onLogin={handleLogin} defaultRegister />;
}
    ...
    return (
        <Routes>
            ...
            <Route path="/app/*" element={
                <AuthGate>
                    <AppProvider>
                        <ProductShell onLogout={handleLogout} />
                    </AppProvider>
                </AuthGate>
            } />
